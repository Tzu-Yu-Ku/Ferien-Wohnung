...
Generell besteht der Anspruch, dass das Pflichtenheft *korrekt*, *komplett* und *konsistent* ist. Da dieses Pflichtenheft bei Abgabe als Maßstab für die Software dient, sollten sämtliche definierten Ziele *erfüllbar* sein. Eine *Rückverfolgung* ist wichtig, da das Pflichtenheft während des gesamten Entwicklungsprozesses verwendet und als Referenz herangezogen wird. Ferner besteht der Anspruch der einfachen Erweiterbarkeit, obgleich Änderungen nach Zustimmung durch die Interessenvertreter auf ein Minimum begrenzt werden sollten. Da Anforderungen und Umstände konstanten Änderungen unterliegen, sind Anpassungen erwartbar und müssen entsprechend dokumentiert werden.
...
- dabei möglichst wenig eigenen (Wartungs-)Aufwand haben
- aufgrund starker Konkurrenz intuitiv benutzbare und ansprechende Benutzeroberfläche darbieten
- möglichst nahtlose Integration der Veranstaltungsfirma
...
- Einfache Integration von Veranstaltungen auf der Detailseite der jeweiligen Ferienwohnung
|Veranstaltungsmitarbeiter |3 |Erstellt und verwaltet Veranstaltungen für seine Firma, erhöht dadurch die Attraktivität der Ferienwohnungen und somit unserer Plattform a|
- intuitive Übersicht über seine Veranstaltungen und deren Bearbeitungsmöglichkeiten
...
Das Kontextdiagramm beschreibt die Integration des geplanten Systems. Berücksichtigt werden alle Nutzergruppen und deren Zugriff auf das System sowie jegliche Software Dritter, welche auf das System zugreift oder auf die durch unser System zugegriffen wird.
...
=== 6.1 Akteure
Akteure sind die Benutzer des Software-Systems oder Nachbarsysteme, welche darauf zugreifen. Die nachfolgende Tabelle listet alle Akteure der Software und deren Beschreibung auf. Lassen sich Akteure zu einer Gruppe zusammenfassen, wird diese _kursiv_ dargestellt.

[options="header"]
[cols="1,4"]
|===
|Name |Beschreibung
|unregisteredUser | Stellt einen nicht registrieren Besucher der Seite dar, der mit dem System interagiert.
...
|Akteure | UserAccount
|Trigger | unregisteredUser drückt auf "Einloggen" in der Navigationsleiste, um mehr Funktionen zu erhalten
|Vorbedingung(en) | UserAccount ist registriert und noch nicht eingeloggt
|Nötige Schritte a| - 1. unregisteredUser drückt auf "Einloggen" in der Navigationsleiste
- 2. unregisteredUser gibt seine Anmeldedaten ein
- 3. unregisteredUser  drückt auf “Einloggen” Button
|Erweiterung | ...
...
|Akteure | UserAccount
|Trigger | UserAccount will das Portal verlassen und drückt deswegen auf "Ausloggen"
|Vorbedingung(en) | UserAccount ist eingeloggt
|Nötige Schritte | UserAccount drückt auf “Ausloggen” in der Navigationsleiste 
|Erweiterung | ...
|benötigte Funktionen | [F0010]
|===
...
|ID | **<<UC0120>>**
|Name | Registrierung
|Beschreibung | unregisteredUser soll die Möglichkeit haben sich als Mieter/Tenant registrieren zu können.
...
|Nötige Schritte a| - 1. unregisteredUser drückt auf registrieren in der Navigation bar
- 2. unregisteredUser gibt die geforderten Daten ein und akzeptiert die Bedingungen
- 3. unregisteredUser drückt auf registrieren
|Erweiterung | ...
...
|Akteure | UserAccount
|Trigger | UserAccount ändert seine persönlichen Daten, indem er auf sein Konto geht, die Änderungen festlegt und bestätigt.
|Vorbedingung(en) | eingeloggt als UserAccount
|Nötige Schritte a| - 1. Auf Konto drücken, um zur Account-Übersicht zu kommen
...
|Beschreibung | UserAccount soll die Möglichkeit haben weitere Details über ein HolidayHome einzusehen
...
|Nötige Schritte a| - 1. Es wird auf Ferienhäuser in der Navigationsleiste gedrückt
|Erweiterung | ...
...
- 3. Tenant wählt einen Zeitpunkt aus
- 4. Tenant kann optional verfügbare Events dazubuchen (UC0220)
- 5. Tenant klickt auf "Buchen"/"Reservieren"
- 6. Tenant bezahlt die Anzahlung
|Erweiterung | ...
...
|Beschreibung | Tenant soll die Möglichkeiten haben, Events, die angeboten werden, dazubuchen
|Akteure | Tenant
...
- 2. Tenant erhält eine Liste möglicher Events und kann diese durch Klicken dazubuchen
...
|Nötige Schritte a| - 1. Man klick in der Navigationsleiste auf den Reiter "Buchungen".
|Erweiterung | ...
...
|Akteure | Host
|Trigger | Host bekommt eine Benachrichtigung über eine offene Buchung. Dieser kann dann ins Buchungsmenü gehen und diese per Klick auf "Bestätigen" bestätigen.
|Vorbedingung(en) | Anzahlung für eine Reservierung ist bei dem Host eingegangen
|Nötige Schritte a| - 1. Der Host klick in der Navigationsleiste auf den Reiter "Buchungen".
...
|Beschreibung | Tenant soll die Möglichkeit haben, Buchungen zu stornieren. Die erfolgte Anzahlung soll erstattet werden.
|Akteure | Tenant
|Trigger | Tenant klickt in der Buchungsübersicht auf den Button "Stornieren"
|Vorbedingung(en) | Tenant hat ein HolidayHome reserviert/gebucht
|Nötige Schritte a| - 1. Tenant klickt in der Navigationsleiste auf den Reiter "Buchungen".
- 2. Wählt eine Buchung aus und klickt bei dieser auf den “Stornieren” Knopf
|Erweiterung | ...
...
|Beschreibung | Host soll die Möglichkeiten haben die schon abgeschlossenen Buchungen gesammelt einsehen zu können.
|Akteure | Host
...
|Nötige Schritte a| - 1. Host klickt in der Navigationsleiste auf den Reiter "Buchungen"
- 2. Host wählt ein Haus und klickt auf "Buchungshistorie ansehen"
|Erweiterung | ...
...
|Beschreibung | Host soll die Möglichkeiten haben, neue HolidayHome auf der Plattform anzubieten
|Akteure | Host
|Trigger | Host geht im Ferienhausbereich auf "neue Wohnung hinzufügen", gibt die Daten zur Wohnung ein und bestätigt alles per Klick auf den Button "Angebot erstellen".
|Vorbedingung(en) | Host ist eingeloggt
|Nötige Schritte a| - 1. Host klickt auf Ferienhäuser in der Navigationsleiste
- 2. Host klickt auf "neues Ferienhaus hinzufügen"
- 3. Host gibt Daten des Ferienhauses ein
- 4. Host bestätigt alles durch Klicken auf den Knopf "Angebot erstellen"
|Erweiterung | ...
...
|[F0020]|v0.1|Registrieren a|Das System soll es dem unregistrierten Nutzer (Tenant) ermöglichen sich zu registrieren bzw. eine Registrierung zu beantragen.
Der jeweiligen Nutzer muss dafür folgende Informationen zur Verfügung stellen:
...
|[F0022]|v0.1|Vermieter registrieren a|Das System soll es dem Administrator ermöglichen, einen neuen Vermieter-Account zu erstellen. Dafür sind folgende Informationen zur Verfügung zu stellen:
...
|[F0023]|v0.1|Eventmitarbeiter registrieren a|Das System soll es dem Administrator ermöglichen, einen neuen Eventmitarbeiter-Account zu erstellen. Dafür sind folgende Informationen zur Verfügung zu stellen:
...
|[F0110]|v0.1|Katalog a|Das System soll Lesezugriff auf eingetragene Häuser, durch einen Katalog, gewähren.
...
|[F0200]|v0.1|Buchung a|Das System soll es dem Nutzer ermöglichen Buchungen wie Einkaufswagen zu benutzen in dem diese, vorübergehend das für den Mieter in Frage
...
|[F0210]|v0.1|Buchung anzeigen a|Das System soll es dem Mieter erlauben seine Buchung anzuschauen, was ihm folgende Daten zurückgeben soll:

* Haus Name
* Buchungszeitraum
* Liste mit zusätzlichen Events jedes mit Einzelpreis aufgelistet
...
|[F0240]|v0.1|Bestellungen a|Das System soll in der Lage sein Bestellungen dauerhaft zu speichern.

|[F0241]|v0.1|Bestellungen erstellen a|Das System soll in der Lage sein Bestellungen aus den Angaben zur Buchung hinaus zu erstellen.
...
Sicherheit beschreibt den Umgang mit personenbezogenen Daten und ähnliches.

Benutzbarkeit beschreibt die Benutzerfreundlichkeit, vor allem für Tenant und Host.
...
Als Eventmitarbeiter kann man sich stets über den "Einloggen"-Knopf in der Kopfzeile der Benutzeroberfläche einloggen (per E-Mail Adresse und Passwort). Über die Startseite oder den Knopf "Ferienhäuser" in der Kopfzeile, kann auch ein Eventmitarbeiter das Ferienhausangebot durchstöbern. Nachdem sich der Eventmitarbeiter eingeloggt hat, kann er über neue Knöpfe in der Kopfzeile seine Eventverwaltung aufrufen oder ein neues Event auf der Plattform bekannt geben. Die Eventverwaltung stellt alle Veranstaltungen der Eventfirma, für die der Eventmitarbeiter arbeitet, in einer Liste dar und über die Liste kann jedes Event über entsprechende Buttons bearbeitet, gelöscht und/oder abgesagt werden.
...
|===
|ID                   |AT0130
|Anwendungsfall        |UC0210 und UC0220
...
|Vorbedingung         |Tanent ist eingeloggt und hat ein HolidayHome gebucht
|Event                |Tanent klickt auf "Buchungen". Er wählt die Buchung aus, die erstornieren will. Dann wählt er "Stornieren" aus.
|Erwartete Resultate a|
- Die Buchung wurde storniert.
- Der Tanent ist nun wieder auf auf der ausgewählten Buchungsseite.
|===

|===
|ID |AT0150
|Anwendungsfall |UC0650
|Vorbedingung |Event wurde von Eventemployee erstellt.
|Event |Eventemployee wählt ein Event aus. Nun drückt er auf "Absagen" und bestätigt dann nochmal mit "Ja".
|Erwartete Resultate a|
- Die Buchung wurde abgesagt.
- Wenn das Event gebucht wurde, dann werden diejenigen informiert, die dieses Event gebucht hatten.
- Das Geld wird zurück überwiesen.
|===
...
|Anwendungsfall |UC0320
|Vorbedingung |Tanent ist eingeloggt und hat ein HolidayHome gebucht
|Event |Tanent klickt auf "Buchungen". Er wählt die Buchung aus, die er stornieren will. Dann wählt er "Stornieren" aus.
|Erwartete Resultate a|
- Die Buchung wurde storniert.
- Der Tanent ist nun wieder auf der ausgewählten Buchungsseite.
|===

== 13.Glossar
Sämtliche Begriffe, die innerhalb des Projektes verwendet werden und deren gemeinsames Verständnis aller beteiligten Stakeholder essenziell ist, sollten hier aufgeführt werden.
